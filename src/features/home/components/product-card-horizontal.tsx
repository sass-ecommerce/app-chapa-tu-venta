...
import { getStockBadgeColor } from '../utils/utils';
...
// Inline styles constants outside component to avoid re-creating on each render
...
export const ProductCardHorizontal = React.memo(
              ...
              {product.priceList > product.price && (
                <View className="flex-1">
                  ...
                  <Text className="text-xs font-semibold text-green-600 dark:text-green-400">
                    {Math.round(((product.priceList - product.price) / product.priceList) * 100)}%
                    off
                  </Text>
                </View>
              )}
  ...
  (prevProps, nextProps) => {
    return (
      prevProps.product.slug === nextProps.product.slug && prevProps.onPress === nextProps.onPress
    );
  }
);
